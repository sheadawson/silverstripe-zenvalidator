...
class ZenValidator extends Validator
{
    ...
    public function applyParsley()
    {
        ...
        $parsleyVersion       = self::config()->parsley_version;
        $parsleyFolder        = 'parsley';
        $parsleyFile          = 'parsley.remote.min.js';
        if ($parsleyVersion) {
            $parsleyFolder = 'parsley-'.$parsleyVersion;
            $parsleyFile   = 'parsley.min.js';

            // We use an up to date version of parsley.
            // Do not include jquery because SilverStripe version is not supported
        } else {
            // We use the default version. Include jquery.
            Requirements::javascript(THIRDPARTY_DIR.'/jquery/jquery.js');
        }

        Requirements::javascript(ZENVALIDATOR_PATH.'/javascript/'.$parsleyFolder.'/'.$parsleyFile);

        $lang = i18n::get_lang_from_locale(i18n::get_locale());
        if ($lang != 'en') {
            Requirements::javascript(ZENVALIDATOR_PATH.'/javascript/'.$parsleyFolder.'/i18n/'.$lang.'.js');
        }
        ...
        if ($this->form) {
            if ($this->defaultJS) {
                ...
                if ($parsleyVersion) {
                    Requirements::javascript(ZENVALIDATOR_PATH.'/javascript/zenvalidator_'.$parsleyVersion.'.js');
                } else {
                ...
                foreach ($constraints as $constraint) {
                    if (!$constraint->validate($data[$fieldName])) {
                        $this->validationError($fieldName,
                            $constraint->getMessage(), 'required');
                        $valid = false;
                    }
                }
            }
